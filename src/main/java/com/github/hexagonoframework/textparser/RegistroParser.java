...
public class RegistroParser {
    ...
    private static void assertClassIsNotNull(Class<?> clazz) {
        if (null == clazz) throw new ParserException(of(ParserErrorType.ERR_PARSE_CLASSE_NULA));
    }
    
    private static <T> void assertRegistroIsNotNull(T objeto) {
        if (null == objeto) throw new ParserException(of(ParserErrorType.ERR_PARSE_REGISTRO_NULO));
    }
}
